...
const UserManagement = () => {
  ...
  async function userinfo() {
    try {
      const response = await axios.get("https://603-bcakend-new.vercel.app/api/v1/users", {
  ...
  const handleEditUser = (updatedUser: User) => {
    setUsers(users.map(u => u.email === updatedUser.email ? updatedUser : u)); // Update user in the list
        ...
        <div>
                ...
                {currentUsers.map((user, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition">
                    ...
                    <td className="py-4 px-6 border-b">
                      <span
                        className={`inline-block px-3 py-1 text-sm font-semibold rounded-full ${user.kyc ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                          }`}
                      >
                    ...
                    <td className="py-4 px-6 border-b">
                      <button
                        onClick={() => handleOpenEditModal(user)} // Open edit modal with selected user
          ...
          <div className="flex justify-between mt-6">
            <button
              ...
              className={`px-6 py-3 rounded-lg shadow-md ${currentPage === 1 ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-blue-600 text-white hover:bg-blue-500 transition duration-300"
                }`}
            >
              Previous
            </button>
            <button
              ...
              className={`px-6 py-3 rounded-lg shadow-md ${currentPage === Math.ceil(filteredUsers.length / usersPerPage)
