...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post(`${PORT}/api/v1/auth/admin/login`, data, {
      ...
      toast.error("Access Denied. Repeated failed login attempts may result in account suspension.");
    } finally {
      setloading(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-3 border rounded-lg text-gray-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 transition-colors duration-300 ${focusedInput === 'email' ? 'ring-2 ring-blue-500' : 'border-gray-300'}`}
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-3 border rounded-lg text-gray-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 transition-colors duration-300 ${focusedInput === 'password' ? 'ring-2 ring-blue-500' : 'border-gray-300'}`}
