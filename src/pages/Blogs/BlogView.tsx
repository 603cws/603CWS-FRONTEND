...
const BlogDetail: React.FC = () => {
    ...
    useEffect(() => {
        if (title) {
            ...
            const foundBlog = blogsData.find(blog => blog.title === decodedTitle && blog.title.replace(/\s/g, "_")); // Find the blog based on category and title
    ...
    return (
        <div className="min-h-screen w-full bg-gray-50">
            ...
            <div className="max-w-4xl mx-auto mt-20 my-12 p-8 shadow-sm rounded-lg">
                ...
                <br />
                <img src={blog.image} alt={blog.title} className="w-full h-72 object-cover rounded-lg mb-6 shadow-md" />
                ...
                <div className="mt-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {blogsData
                            .filter((relatedBlog) => relatedBlog.category === blog.category && relatedBlog.title !== blog.title)
                            .slice(0, 3)
                            .map((relatedBlog, index) => (
                                <div key={index} className="bg-white shadow-md rounded-lg p-4" onClick={() => {
                                    navigate("/blogs"); navigate(`/blogs/${relatedBlog.title.replace(/\s/g, "_")}`);
                                }}>
                                    <img src={relatedBlog.image} alt={relatedBlog.title} className="w-full h-40 object-cover rounded-md mb-2" />
                                    <h3 className="font-bold text-base text-gray-800">{relatedBlog.title.substring(0, 30)}...</h3>
                                </div>
                            ))}
                    </div>
                </div>
