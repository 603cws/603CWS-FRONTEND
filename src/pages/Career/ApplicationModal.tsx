...
const ApplicationModal: React.FC<ApplicationModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async(event: React.FormEvent) => {
        ...
        try {
            const response = await axios.post(`${PORT}/api/v1/career/send`, { name, email, phone, position, experience }, {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
            <div className="bg-white rounded-lg shadow-xl p-8 max-w-md w-full max-h-screen overflow-y-auto">
                <h2 className="text-2xl font-semibold text-gray-800 mb-6">Apply for the Position</h2>
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        <label className="block text-gray-700 font-medium mb-2" htmlFor="position">Applying For</label>
                        <select
                            id="position"
                            value={position}
                    ...
                    <div className="flex justify-between space-x-3">
                        <button
                            type="submit"
                            className={`w-full py-3 ${isFormValid ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-gray-300'} text-white rounded-lg font-semibold transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-yellow-400`}
