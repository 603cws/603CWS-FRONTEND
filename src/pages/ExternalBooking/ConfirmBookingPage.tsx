...
import { FaCalendarAlt, FaShoppingCart, FaWifi, FaBriefcase, FaCoffee } from 'react-icons/fa'; // Import FontAwesome icons
...
const ConfirmPayment = () => {
  ...
  const getFilteredTimes = (selectedDay: number | null, currentTime: Date): string[] => {
    if (selectedDay === null) {
      return times;
    }

    const today = new Date();
    const selectedDate = new Date(currentTime.getFullYear(), currentTime.getMonth(), selectedDay);
  ...
  useEffect(() => {
    ...
    const foundLocation = locations.flatMap(city => city.locations)
  ...
  useEffect(() => {
    setCartTotal(bookings.reduce((total, booking) => total + booking.price, 0) +
      dayPasses.reduce((total, dayPass) => total + dayPass.price, 0))
  }, [bookings, dayPasses])
  ...
  const dayStyle: React.CSSProperties = {
    width: windowWidth > 1280 ? '60px' : windowWidth > 545 ? '45px' : windowWidth > 377 ? '40px' : '35px',
  ...
  const changeday = (day: number) => {
    ...
    if (timings.length > 0) {
      ...
      timings.forEach(([start, end]) => {
        ...
        availableStartTimesUnfiltered = availableStartTimesUnfiltered.filter(time => {
          const timeInMinutes = getTimeInMinutes(time);
  ...
  const selectendtimefunction = (starttime: string) => {
    ...
    for (let i = startIndex + 1; i < availableStartTimes.length; i++) {
      ...
      const differenceInMinutes = currentEndTimeInMinutes - previousTimeInMinutes;
  ...
  const fetchLocationDayPassBookings = async () => {
    try {
      ...
      const response = await axios.post(`${PORT}/api/v1/daypass/getdata`, { selectedLocation, selectedYear: currentYear, selectedMonth: currentMonth + 1 }, {
        withCredentials: true,
    ...
    <div className="font-sans min-h-screen bg-gray-50 relative min-w-full">
      ...
      <div className="container mx-auto px-4 sm:px-6 mt-8 flex flex-col lg:flex-row gap-8 pt-20">
        <div className="lg:w-3/5">
          ...
          <section className="mt-8">
            <h3 className="text-2xl font-bold mb-4 text-gray-800">Building Amenities</h3>
        ...
        {locationDetails?.enablebooking ?
          <div className="lg:w-2/5 rounded-lg p-4 xl:p-6 shadow-xl h-auto">
            <h3 className="text-xl font-bold text-gray-800 flex items-center ">
              <FaCalendarAlt className="mr-2 text-yellow-500" /> Select your preference
            </h3>
            <div className='flex items-center justify-between mt-6'>
              <div className="py-5">
                <p className="text-gray-700 font-semibold">Conference Rooms</p>
                <p className="text-2xl font-semibold text-yellow-500">₹{locationDetails?.conferenceroom}/Hr</p>
              </div>
              {!showcalenderconfroom && (
                ...
                >
                  ...
                  {locationDetails?.conferencerooms?.map((room, index) => (
                    <option key={index} value={room} onClick={() => { setspacetype("conference"); setselectedLocation(room) }}>
                      {room}
                    </option>
                  ))}
                </select>
              ...
              {showcalenderconfroom && (
                <button
                  ...
                  onClick={() => { setshowcalenderconfroom(false); setSelectedDate(''); setselectedLocation("") }}
                >
                  Remove
                </button>
              )}
              ...
              {showcalenderconfroom && (
                <>
                  ...
                  <div style={calendarHeaderStyle}>
                    <button style={buttonStyle} onClick={() => changeMonth(-1)}>{"<"}</button>
                    <h2 style={timestyle}>{`${new Date(currentYear, currentMonth).toLocaleString('default', { month: 'long' })} ${currentYear}`}</h2>
                    <button style={buttonStyle} onClick={() => changeMonth(1)}>{">"}</button>
                  </div>
                  <div style={calendarStyle}>
                    {Array.from({ length: daysInMonth(currentMonth, currentYear) }, (_, index) => index + 1).map(day => (
                      <div
                        key={day}
                        style={{
                          ...dayStyle,
                          ...(isPastDay(day) ? pastDayStyle : isWeekend(day, currentMonth, currentYear) ? partiallyBookedStyle : availableStyle1),
                  ...
                  {showcalenderconfroom && enableconftime && (
                    <div className='px-5 pb-4'>
                      <div style={inputContainerStyle}>
                        ...
                        <select value={selectedStartTime} onChange={handleStartTimeChange} style={inputStyle}>
                      ...
                      <div style={inputContainerStyle}>
                        ...
                        <select value={selectedEndTime} onChange={handleEndTimeChange} style={inputStyle} disabled={!selectedStartTime}>
                      ...
                      {selectedEndTime && (
                        <button
                          ...
                          onClick={() => {
                            setshowcalenderconfroom(false), setshowcalenderdaypass(false); setshowcalendermeetroom(false);
                            addNewBooking({
                              spaceName: selectedLocation,
                              startTime: selectedStartTime,
                              endTime: selectedEndTime,
                              date: selectedDate,
                              price: locationDetails?.conferenceroom * timedifference,
                            })
                          }}
                        >
                          Add <span className='text-white font-extrabold'>₹{locationDetails?.conferenceroom * timedifference}</span>
                        </button>
                      )}
                    </div>)}
                </>
            ...
            <div className='flex items-center justify-between border-t-2 border-gray-200'>
              <div className="py-5">
                ...
                <p className="text-2xl font-semibold text-yellow-500">₹{locationDetails?.meetingroom}/Hr</p>
              </div>
              {!showcalendermeetroom && (
                ...
                >
                  ...
                  {locationDetails?.meetingrooms?.map((room, index) => (
                    <option key={index} value={room} onClick={() => { setspacetype("meeting"); setselectedLocation(room) }}>
                      {room}
                    </option>
                  ))}
                </select>
              ...
              {showcalendermeetroom && (
                <button
                  className=" text-red-600 px-4 py-2 bg-slate-200 rounded-md shadow-lg transition transform hover:bg-gray-300 hover:scale-105"
                  onClick={() => { setshowcalendermeetroom(false); setSelectedDate('') }}
                >
                  Remove
                </button>
              )}
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            addNewBooking({
                              ...
                              endTime: selectedEndTime,
                              date: selectedDate,
                              price: locationDetails?.meetingroom * timedifference,
                            })
                          }}
                        >
                          Add <span className='text-white font-extrabold'>₹{locationDetails?.meetingroom * timedifference}</span>
                        </button>
                      )}
                    </div>)}
                </>
            ...
            <div className='flex items-center justify-between border-t-2 border-gray-200'>
              <div className="py-5">
                ...
                <p className="text-2xl font-semibold text-yellow-500">₹{locationDetails?.daypass}/Day</p>
              </div>
              {!showcalenderdaypass && (
                <select
                  className="bg-gray-200 text-gray-700 px-2 py-2 rounded-md shadow-lg hover:cursor-pointer w-32 md:w-52"
              ...
              {showcalenderdaypass && (
                <button
                  className="text-red-600 px-4 py-2 bg-slate-200 rounded-md shadow-lg transition transform hover:bg-gray-300 hover:scale-105"
                  onClick={() => { setshowcalenderdaypass(false); setSelectedDate(""); setselectedLocation("") }}
                >
                  Remove
                </button>
              )}
                ...
                <>
                  ...
                  <div style={calendarStyle}>
                    {Array.from({ length: daysInMonth(currentMonth, currentYear) }, (_, index) => index + 1).map(day => (
                      <div
                        key={day}
                        aria-disabled={isPastDay(day) || !unavailabledaypasses.includes(day)}
                        style={{
                          ...dayStyle,
                          ...((isPastDay(day) || unavailabledaypasses.includes(day)) ? pastDayStyle : isWeekend(day, currentMonth, currentYear) ? partiallyBookedStyle : availableStyle1),
                          ...(selectedDay === day && selectedStyle),
                        }}
                        onClick={() => {
                          if (!isPastDay(day) && !unavailabledaypasses.includes(day)) {
                            setSelectedDay(day);
                            setSelectedStartTime("");
                            setSelectedEndTime("");
                  ...
                  <div className='w-full flex justify-center'>
                    <input type="text" className='flex justify-center w-32 bg-yellow-200 px-3 py-2 rounded-md shadow-lg border-gray-200' value={selectedDay ? selectedDate : "Select Date"} />
                  </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        bookDayPass({
                          ...
                          month: currentMonth,
                          year: currentYear
                        }); setshowcalenderconfroom(false); setshowcalenderdaypass(false); setshowcalendermeetroom(false); console.log(dayPasses)
                      }}
                    >
                      Add <span className='text-white font-extrabold'>₹{locationDetails?.daypass}</span>
                    </button>
                  )}
                </>
              )}
          ...
          <div className="lg:w-2/5 rounded-lg p-6 shadow-xl h-auto flex justify-center">
            <h1 className='text-red-600 text-base'>Booking is currently unavailable.</h1>
          </div>
        }
      </div>
      ...
      <div className="fixed bottom-0 left-0 w-full rounded-t-3xl bg-stone-700 shadow-2xl px-6 py-3 border-t border-gray-400 z-[50]">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <button onClick={() => navigate("/payment")} className="bg-yellow-500 text-white px-6 py-2 rounded-md shadow-lg transition transform hover:bg-yellow-600 hover:scale-105">
            Proceed
          </button>
        </div>
      </div>
      ...
      <div className="container mx-auto mt-16 p-6 bg-white rounded-lg shadow-lg">
        <h4 className="text-2xl font-bold text-gray-800 mb-4">Office Location</h4>
